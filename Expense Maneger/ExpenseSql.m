...
+(void)addExpense:(sqlite3 *)database exp:(Expense*)exp{
  ...
  /*
    if (sqlite3_prepare_v2(database,[query UTF8String],-1,&statment,nil) == SQLITE_OK){
        ...
        //LIROT EH LAAVOD IM double
        sqlite3_bind_text(statment, 4, [exp.examount UTF8String],-1,NULL);
...
+(NSArray*)getExpenses:(sqlite3 *)database{
    ...
    if (sqlite3_prepare_v2(database,"SELECT * from EXPENSES;", -1,&statment,nil) == SQLITE_OK){
        while(sqlite3_step(statment) == SQLITE_ROW){
            ...
            // examount - double
            NSNumber* examount = [NSNumber numberWithLong:(long)sqlite3_column_text(statment,3)];
            NSDate* exdate = [NSDate date:(exdate)sqlite3_column_text:(statment,4)];
            NSString* eximage = [NSString stringWithFormat:@"%s",sqlite3_column_text(statment,5)];
            
            Expense* exp = [[Expense alloc] init:timeInMillisecond exname:exname excategory:excategory examount:examount exdate:exdate eximage:eximage];
            [data addObject:exp];
        }
    }else{
        NSLog(@"ERROR: addExpense failed %s",sqlite3_errmsg(database));
